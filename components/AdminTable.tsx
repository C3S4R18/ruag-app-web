...
export default function AdminTable({ onOpenChat, refreshTrigger = 0, onNotifyChange, tipoFilter = 'obrero' }: AdminTableProps) {
  ...
  const getActionStyle = (actionMsg: string) => {
      ...
      if (msg.includes('cesó') || msg.includes('ceso') || msg.includes('cesado')) return { icon: <AdminGifIcon name="ceso.gif" size={28} variant="bare"/>, bg: 'bg-white', text: 'text-red-600', border: 'border-red-200' };
      if (msg.includes('eliminó') || msg.includes('baja')) return { icon: <Trash2 size={14}/>, bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-200' };
